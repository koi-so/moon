...
#define DEBUGBREAK EASTL_DEBUG_BREAK()
...
#define ASSERT(isFalse, ...)                                                   \
  if (!(bool)(isFalse)) {                                                      \
    DXUtility::Print("\nAssertion failed in " STRINGIFY_BUILTIN(               \
        __FILE__) " @ " STRINGIFY_BUILTIN(__LINE__) "\n");                     \
...
#define ASSERT_SUCCEEDED(expr, ...)                                            \
  {                                                                            \
    ...
    if (CUSTOM_FAILED(hr)) {                                                   \
                                                                               \
      DXUtility::Print("\nHRESULT failed in " STRINGIFY_BUILTIN(               \
          __FILE__) " @ " STRINGIFY_BUILTIN(__LINE__) "\n");                   \
...
#define WARN_ONCE_IF(isTrue, ...)                                              \
  {                                                                            \
    ...
    if ((bool)(isTrue) && !s_TriggeredWarning) {                               \
      ...
      DXUtility::Print("\nWarning issued in " STRINGIFY_BUILTIN(               \
          __FILE__) " @ " STRINGIFY_BUILTIN(__LINE__) "\n");                   \
...
#define ERROR(...)                                                             \
  DXUtility::Print("\nError reported in " STRINGIFY_BUILTIN(                   \
      __FILE__) " @ " STRINGIFY_BUILTIN(__LINE__) "\n");                       \
  DXUtility::PrintSubMessage(__VA_ARGS__);                                     \
  DXUtility::Print("\n");
