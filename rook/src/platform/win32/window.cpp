...
static auto GetScreenCenteredPosition(const Extent2D &size) -> Offset2D {
  return {GetSystemMetrics(SM_CXSCREEN) / 2 -
              static_cast<int32_t>(size.width / 2),
          GetSystemMetrics(SM_CYSCREEN) / 2 -
              static_cast<int32_t>(size.height / 2)};
}
...
auto Win32Window::CreateWindowHandle(const WindowAttributes &attributes)
  ...
  auto converted_count = MultiByteToWideChar(
      ...
      static_cast<int32_t>(attributes.title.length()), title, sizeof(title));
  ...
  EASTL_ASSERT_MSG(window, "Failed to create window");
