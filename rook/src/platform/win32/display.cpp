...
struct Win32DisplayContainer {
  ...
  Win32DisplayContainer(eastl::unique_ptr<Win32Display> &&display,
                        uint32_t cache_index)
      : display{std::move(display)}, cache_index{cache_index} {}

  eastl::unique_ptr<Win32Display> display{nullptr};
  uint32_t cache_index{0};
};

static eastl::vector<Win32DisplayContainer> g_display_list;
static eastl::vector<Win32Display *> g_display_ref;
...
static auto CALLBACK Win32MonitorEnumProcedure(HMONITOR monitor, HDC hDC,
  ...
  if (it != g_display_list.end()) {
    ...
    auto display = eastl::make_unique<Win32Display>(monitor);
...
auto Display::count() -> size_t {
  UpdateDisplayList();
  return g_display_list.size();
}

auto Display::get_list() -> Display *const * {
...
auto Display::get(size_t index) -> Display * {
  UpdateDisplayList();
...
auto Display::is_cursor_visible() -> bool {
  bool visible;
  if (rook::platform::IsCursorVisible(visible))
    return visible;
  return false;
}
...
auto Win32Display::get_device_name() const -> eastl::string {
  ...
  return std::move(
      eastl::string{device_name, static_cast<size_t>(device_name_size)});
}

auto Win32Display::get_offset() const -> Offset2D {
  MONITORINFO info;
  get_info(info);
  return {static_cast<int32_t>(info.rcMonitor.left),
...
auto Win32Display::get_display_mode_list() const
    -> eastl::vector<DisplayAttributes> {
